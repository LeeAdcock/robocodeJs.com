...
- `test/simulationIntegration.test.ts` — the **full sandbox + simulation path**: a real bot is compiled into a real isolate, then driven through `Simulation.run` tick by tick (manually advancing the clock, with a short settle for the now-async handlers) and the resulting tank state asserted — acceleration/movement, rotation to target, firing a travelling bullet, a crashing bot being killed, a tick-driven `setTimeout`, and one tank destroying another with sustained fire. This is the live game loop minus the 100ms interval and the database. Note turret orientation starts random, so tests that fire set `turret.orientation`/`orientationTarget` explicitly for a deterministic aim.
