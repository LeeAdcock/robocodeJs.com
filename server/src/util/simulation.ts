    ...
    arena.processes.forEach(process => {
      process.tanks.forEach((tank, tankIndex) => {
        if (tank.health > 0) {
          ...
          const newX = tank.x + tank.speed * Math.sin(-tank.orientation * (Math.PI / 180))
          const newY = tank.y + tank.speed * Math.cos(-tank.orientation * (Math.PI / 180))
          let collided = false
          ...
          if (newX < 16 || newX > arena.width - 16 || newY < 16 || newY > arena.height - 16) {
            ...
            tank.stats.timesCollided += 1
            if (tank.handlers[Event.COLLIDED]) {
              tank.handlers[Event.COLLIDED]({ angle: normalizeAngle(tank.orientation) })
            }
          }
