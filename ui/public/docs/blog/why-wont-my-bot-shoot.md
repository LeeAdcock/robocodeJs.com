...
"my bot just sits there and never fires." It's frustrating because a bot that won't
...
things, and you can tell which one from the bot's own console output. So before we go
...
lines in and let the bot tell you what it's thinking.
...
that bot stops responding entirely: no moving, no shooting. A crashed bot
looks exactly like a lazy one. Check the log for an error or an [error code](/error-codes);
a thrown exception or a timeout will show up there. If the bot went inert all at once
...
bots that all run this same code. If you wrote a "don't shoot if a friend is near the
line" guard (a good instinct!), double-check it isn't firing constantly with five of your
own bots milling around. A too-strict safety check can silence a gun as effectively as a
bug. Log the reason you're holding fire so you can see how often it triggers.

## Reading the log like a doctor

Notice the pattern in all five: I didn't guess, I asked the bot. Each culprit has a
...
you stop staring at the arena and start reading what the bot is telling you. Get the
