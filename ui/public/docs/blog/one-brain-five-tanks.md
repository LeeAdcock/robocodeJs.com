...
Your app doesn't control _one_ bot. It controls **five**. You write one program, and the
game runs a copy of it on each of your five bots: same code, five bodies, all fighting
at once. This trips up almost everyone at first, and once it clicks, a lot of surprising
behavior stops being surprising. Why does my bot keep bumping into an identical bot?
...
you call `bot.getX()`, you get _that particular bot's_ position, the one currently
running the code. So a line like "drive toward the center" makes all five converge, while
"drive away from my nearest teammate" makes them spread out, because each bot evaluates
"my nearest teammate" from its own spot.

That's the mental shift: stop writing instructions for a single bot and start writing
instructions that produce good behavior no matter which of the five is asking. The best
squads don't hard-code "bot 1 does this, bot 2 does that." They write one rule that,
...
simple broadcast messages: every bot shouts its own position ten times a second, everyone steers
...
follows them. Five bots is the friendly version of that problem, and it's the part of
this game I find hardest to put down.

## Talking to yourself

Here's the part that surprises people: even though all five bots run the same code, each
one is on its own. One bot can't peek at another's variables or read what its radar saw.
The only way they share anything is by talking: `bot.send` broadcasts a message, and any
bot listening for `Event.RECEIVED` hears it. So if one bot's radar spots an enemy, it
...
of your own bots clustered together is five chances to shoot yourself in the back.

This changes how you think about both formation and firing. Clumping your bots feels
...
Spacing is self-preservation, not just board control. And before any bot pulls
...
The teams that win consistently treat their five bots as a group that _spreads out,
shares what it sees, and checks its line before firing_. That's three habits, and none of
them are complicated. They just require remembering, always, that you're not one bot.
...
[teamwork lesson](/learn/teamwork) to tie it together. If your bots keep shooting each
