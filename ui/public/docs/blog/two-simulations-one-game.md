...
draw each server update the instant it arrived, a bot crossing the arena would jump ten
...
continuous. Ten discrete snapshots a second, with hard edges, is not that. A bot doing
...
runs its own little copy of the movement math and _predicts_ where each bot should be.
The server said a bot was here, moving this direction, at this speed. So a sixtieth of
...
is invisible. You just see a bot gliding across the arena, which is exactly the lie I
...
This is the part that keeps me honest. If the browser is going to predict where a bot
...
a bot sliding one way, then the server update says "actually, no," and the bot visibly
