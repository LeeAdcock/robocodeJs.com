...
The turret and radar work differently again: each is measured relative to whatever it is mounted on — the turret's orientation relative to the body, the radar's relative to the turret. Since a contact's `angle` is _also_ relative to the body, that is already the turret's frame, so `bot.turret.setOrientation(contact.angle)` aims straight at the contact with no conversion. To point at a place rather than along a bearing, `bot.turnTowards(x, y)` — and the turret and radar versions — take coordinates and do the arithmetic for you.
