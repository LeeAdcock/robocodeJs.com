...
- `bot.on(Event.DEATH, () => {})` Registers a handler that is executed once when the bot is eliminated — whether shot down, destroyed in a collision, or ground down by sudden-death decay. It is your bot's last chance to run code: use it to write final diagnostics to the console (accumulated stats, the last enemy positions you tracked, why the bot thinks it lost) so that behaviour and arena data are captured for debugging after the bot has stopped. The action commands all fail from here — the bot is already dead — so keep the handler to logging and cleanup: movement (`setSpeed`, `turn`, `setOrientation`), turret and radar turns, `bot.turret.fire()`, `bot.turret.radar.scan()`, and `bot.send()` all reject, and `isReady()` on the turret and radar reports `false`. Read-only calls (`getX`, `getHealth`, `clock.getTime`, `arena.createMarker`, and the like) still work, so you can record a final snapshot. A bot that is killed by its own crash does **not** receive `DEATH` (its sandbox has already faulted; the crash itself is on the fault feed).
...
- `bot.send(message) : Promise` Broadcasts a message that every other bot in the arena (teammates **and** enemies) can receive via the `RECEIVED` event. `message` can be a primitive (number, string, boolean, null) or a nested array/object of those primitives (functions, class instances, and other non-JSON values cannot be sent). There are no private channels: to coordinate a team, tag your messages with something teammates recognize and validate incoming messages before acting on them. A message that isn't JSON data, is larger than 4,096 characters once encoded, or nests more than 8 levels deep is rejected. `send` throws (code `E023`). A bot may also broadcast at most 50 messages per clock tick. `send` returns a promise that resolves once the broadcast has gone out, or that is rejected (code `E024`) when that budget is already spent — in which case the message reached nobody. Awaiting the send, or attaching a `.catch()`, is how you tell a delivered broadcast from a dropped one; ignoring the promise is safe — an over-budget send you never await drops quietly and never crashes your bot. See the [error code reference](/error-codes).
