...
const roadBuilderVert = (
  ...
  if (isSandTransition || isStraight) {
    ...
    if (oldX + 1 === x)
      terrain[0][x][y] = (
        <image
          href={`/sprites/tile${isSand(x, y) ? 'Sand' : 'Grass'}_roadEast.png`}
          x={32 * x}
          y={32 * y}
          height={32}
          width={32}
...
const roadBuilderHoriz = (
  ...
  if (isSandTransition || isStayStraight) {
    ...
    if (oldY + 1 === y)
      terrain[0][x][y] = (
        <image
          href={`/sprites/tile${isSand(x, y) ? 'Sand' : 'Grass'}_roadNorth.png`}
          x={32 * x}
          y={32 * y}
          height={32}
          width={32}
...
const generateTerrain = () => {
  ...
  terrain[0].unshift(
    <rect opacity=".05" height="100%" width="100%" fill="url(#shadedRelief)" />
  );
