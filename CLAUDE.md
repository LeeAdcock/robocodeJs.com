...
`server/src/api/mcp.ts` is an in-process **Model Context Protocol** server at `POST /api/mcp` (Streamable HTTP, stateless, bearer-token auth) exposing ~23 tools (bot CRUD + `check_bot_source`/compile/reboot; arena create/delete/control incl. `set_arena_speed`/`set_arena_seed`; `arena_status`, `match_summary`; `recent_logs`/`recent_faults`; the global `leaderboard`), **resources** (bot docs, `robocode.d.ts`, samples, error codes), and **prompts** (`write_bot`, `debug_bot`, `run_match`). Almost every tool acts only on the token owner's own resources (`ownedApp`/`ownedArena` mirror the REST ownership checks) — keep the MCP caps in sync with the REST caps by hand; the `leaderboard` tool is the one deliberately global (public ranking data). User-facing setup guide at `/mcp` (`ui/public/docs/mcp.md`).
