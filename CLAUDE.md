...
Both packages use **Vitest** (`npm test` runs `vitest run`, `npm run test:watch` for watch mode). Tests live in each package's `test/` directory (outside `src`, so they're excluded from the `tsc` build). Server coverage spans the simulation physics (`simulation.test.ts`), tick-driven timers (`scheduleFactory.test.ts`), the `Tank`/`Turret`/`Radar` classes (`tankTypes.test.ts`), the API endpoints with auth/ownership (`api.test.ts`, `auth.test.ts`, `session.test.ts`), the Postgres services (`services.test.ts`), name generation (`nameFactory.test.ts`), and **real-isolate** sandbox integration (`compiler.test.ts`); the UI covers the SSE reducer (`arenaReducer.test.ts`), shared geometry (`geometry.test.ts`), and client-side interpolation (`simulate.test.ts`). Two patterns recur: `Simulation.run` is driven with lightweight mock tanks (it only invokes `tank.handlers[...]` and mutates plain fields, so no isolates are needed), while `compiler.test.ts` spins up a real `isolated-vm` isolate to lock the bot-facing contract. DB-/isolate-coupled modules `vi.mock('../src/util/db', ...)` (or mock the relevant services) at the top of the file.
